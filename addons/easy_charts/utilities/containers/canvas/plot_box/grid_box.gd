...
var _x_label_y_offset_multiplier: int = 1
...
func _get_x_tick_label_position(base_position: Vector2, text: String, x_pixel_dist: float) -> Vector2:
	...
	if get_parent().chart_properties.y_offset_x_labels:
		_x_label_y_offset_multiplier = posmod(_x_label_y_offset_multiplier + 1, 2)
		return  base_position + Vector2(
			- get_parent().chart_properties.font.get_string_size(text).x / 2 + x_offset,
			ThemeDB.fallback_font_size + get_parent().chart_properties.x_tick_size + _x_label_y_offset_multiplier * 15.0
		)
	else:
		return  base_position + Vector2(
			- get_parent().chart_properties.font.get_string_size(text).x / 2 + x_offset,
